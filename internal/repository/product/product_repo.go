...
func (pr *ProductRepo) CreateProduct(product *model.Product, categoryIDs []int64) (*model.Product, error) {
	...
	query := `INSERT INTO products(name, slug, thumbnail_url, short_description, description, brand, status, is_published, published_at, min_price, price_override, discount_percent)
              VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`

	res, err := tx.Exec(query, product.Name, product.Slug, product.ThumbnailURL, product.ShortDescription, product.Description, product.Brand, product.Status, product.IsPublished, product.PublishedAt, product.MinPrice, product.PriceOverride, product.DiscountPercent)
	if err != nil {
		tx.Rollback()
...
func (pr *ProductRepo) BulkCreateProducts(products []*model.Product, categoryIDsMapping [][]int64) error {
	...
	for i, product := range products {
		res, err := stmt.Exec(product.Name, product.Slug, product.ThumbnailURL, product.ShortDescription, product.Description, product.Brand, product.Status, product.IsPublished, product.PublishedAt, product.MinPrice, product.PriceOverride, product.DiscountPercent)
		if err != nil {
			tx.Rollback()
...
func (pr *ProductRepo) GetProductByID(id int64) (*model.Product, error) {
	...
	query := `SELECT id, name, slug, thumbnail_url, short_description, description, brand, status, is_published, published_at, min_price, price_override, discount_percent, avg_rating, rating_count, created_by, updated_by, created_at, updated_at, deleted_at
			  FROM products 
			  WHERE id=? AND deleted_at IS NULL`

	rows := pr.DB.QueryRow(query, id)
	var product model.Product
	err := rows.Scan(&product.ID, &product.Name, &product.Slug, &product.ThumbnailURL, &product.ShortDescription, &product.Description, &product.Brand, &product.Status, &product.IsPublished, &product.PublishedAt, &product.MinPrice, &product.PriceOverride, &product.DiscountPercent, &product.AvgRating, &product.RatingCount, &product.CreatedBy, &product.UpdatedBy, &product.CreatedAt, &product.UpdatedAt, &product.DeletedAt)
	if err != nil {
		return nil, err
	}
	return &product, nil
...
func (pr *ProductRepo) GetManyProduct(ids []int64) ([]model.Product, error) {
	...
	query := fmt.Sprintf(`
        SELECT id, name, slug, thumbnail_url, short_description, description, brand, status, is_published, published_at, min_price, price_override, discount_percent, avg_rating, rating_count, created_by, updated_by, created_at, updated_at, deleted_at
        FROM products 
...
func (pr *ProductRepo) SearchProducts(req *model.SearchProductsRequest) ([]model.Product, int, error) {
	...
	if req.MinPriceFilter != nil {
		whereClauses = append(whereClauses, `(
			COALESCE(
				(SELECT MIN(pv.price_override) FROM product_variants pv WHERE pv.product_id = p.id AND pv.is_active = 1 AND pv.price_override IS NOT NULL),
				COALESCE(p.price_override, p.min_price)
	...
	switch req.SortBy {
	case "price":
		orderBy = "COALESCE(p.price_override, (p.min_price * (1 - p.discount_percent / 100)))"
	case "rating":
	...
	baseQuery := `
		SELECT p.id, p.name, p.slug, p.thumbnail_url, p.short_description, p.description, p.brand,
		       p.status, p.is_published, p.published_at, p.min_price, p.price_override, p.discount_percent,
	...
	for rows.Next() {
		...
		err := rows.Scan(
			...
			&p.MinPrice, &p.PriceOverride, &p.DiscountPercent, &p.AvgRating, &p.RatingCount,
...
func (pr *ProductRepo) UpdateProduct(product *model.Product, categoryIDs []int64) (*model.Product, error) {
	...
	query := `UPDATE products 
              SET name=?, slug=?, thumbnail_url=?, short_description=?, description=?, brand=?, status=?, is_published=?, published_at=?, min_price=?, price_override=?, discount_percent=?, updated_at=NOW()
              WHERE id=? AND deleted_at IS NULL`

	res, err := tx.Exec(query, product.Name, product.Slug, product.ThumbnailURL, product.ShortDescription, product.Description, product.Brand, product.Status, product.IsPublished, product.PublishedAt, product.MinPrice, product.PriceOverride, product.DiscountPercent, product.ID)
	if err != nil {
		tx.Rollback()
...
func (pr *ProductRepo) GetAllProductsSoftDeleted() ([]model.Product, error) {
	rows, err := pr.DB.Query("SELECT id, name, slug, thumbnail_url, short_description, description, brand, status, is_published, published_at, min_price, price_override, discount_percent, avg_rating, rating_count, created_by, updated_by, created_at, updated_at, deleted_at FROM products where status='archived' AND deleted_at IS NOT NULL")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var product model.Product
		if err := rows.Scan(&product.ID, &product.Name, &product.Slug, &product.ThumbnailURL, &product.ShortDescription, &product.Description, &product.Brand, &product.Status, &product.IsPublished, &product.PublishedAt, &product.MinPrice, &product.PriceOverride, &product.DiscountPercent, &product.AvgRating, &product.RatingCount, &product.CreatedBy, &product.UpdatedBy, &product.CreatedAt, &product.UpdatedAt, &product.DeletedAt); err != nil {
			return nil, err
		}
		products = append(products, product)
	}
